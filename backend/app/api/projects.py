...
def list_projects(limit: int = 100, offset: int = 0, db: Session = Depends(get_db)) -> list[Project]:
    return projects_service.list_projects(db, limit=limit, offset=offset)
...
def update_project(project_id: uuid.UUID, data: ProjectUpdate, db: Session = Depends(get_db)) -> Project:
